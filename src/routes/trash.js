...
router.get('', (request, response) => {
  if (request.query.position) {
    ...
    const sql = 'SELECT trash.*, ROUND((111.111 * '
    ...
    // 'LIMIT 15;';
    const parameters = [latitude, longitude, latitude, 0, Number.parseFloat(process.env.DISTANCE_THRESHOLD)];
...
router.post('', (request, response) => {
  ...
  for (const neededField of neededFields) {
    ...
    } if (['latitude', 'longitude'].includes(neededField)) {
  ...
  const sql = 'INSERT INTO trash(trashID, imagePath, latitude, longitude, comment, flag, username) '
              + 'VALUES(null, ?, ?, ?, ?, ?, ?)';
